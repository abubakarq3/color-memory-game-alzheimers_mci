...
public class LevelfiveColorchanger : MonoBehaviour
{
    public GameObject cylinder;                // Referencia al cilindro
    public GameObject cube;                    // Referencia al cubo
    public GameObject cube2;                   // Referencia al segundo cubo
    ...
    private Material correctCube2Color;        // Color objetivo para el segundo cubo
    private Material correctSphereColor;       // Color objetivo para la esfera
    private Material correctCapsuleColor;      // Color objetivo para la cápsula
    ...
    {
        // Asignar colores aleatorios a todos los objetos
        ...
        correctCube2Color = colorMaterials[Random.Range(0, colorMaterials.Length)];
        correctSphereColor = colorMaterials[Random.Range(0, colorMaterials.Length)];
        correctCapsuleColor = colorMaterials[Random.Range(0, colorMaterials.Length)];

        cylinder.GetComponent<Renderer>().material = correctCylinderColor;
        cube.GetComponent<Renderer>().material = correctCubeColor;
        cube2.GetComponent<Renderer>().material = correctCube2Color;
        sphere.GetComponent<Renderer>().material = correctSphereColor;
        capsule.GetComponent<Renderer>().material = correctCapsuleColor;

        Debug.Log($"Colores objetivo - Cilindro: {correctCylinderColor.color}, Cubo: {correctCubeColor.color}, Segundo Cubo: {correctCube2Color.color}, Esfera: {correctSphereColor.color}, Cápsula: {correctCapsuleColor.color}");
    ...
    IEnumerator EndRound()
    {
        ...
        bool isCube2Correct = cube2.GetComponent<Renderer>().material.color == correctCube2Color.color;
        bool isSphereCorrect = sphere.GetComponent<Renderer>().material.color == correctSphereColor.color;
        bool isCapsuleCorrect = capsule.GetComponent<Renderer>().material.color == correctCapsuleColor.color;

        if (isCylinderCorrect && isCubeCorrect && isCube2Correct && isSphereCorrect && isCapsuleCorrect)
        {
